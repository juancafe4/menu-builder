	...
	var Splash = _wrapComponent('_component2')(_react3.default.createClass({
	  ...
	  newRestaurant: function newRestaurant(newRestObj) {
	    ...
	    fetch(url, {
	      ...
	      body: JSON.stringify(newRestObj)
	    }).then(function (Response) {
	      return Response.json();
	    }).then(function (data) {
	      console.log('data.id:', data.id);
	      _this.setState({ restaurantId: data.id });
	      _reactRouter.browserHistory.push('/home/' + data.id);
	    }).catch(function (err) {
	      console.log('err:', err);
	    });
	  },
	  restaurantValidation: function restaurantValidation(e) {
	    ...
	    fetch(url).then(function (Response) {
	      ...
	      var newRestaurant = data.filter(function (val) {
	        return val.name === _this2.state.name;
	      });

	      if (newRestaurant.length !== 0) {
	        _this2.setState({ restaurantId: newRestaurant[0].id });
	        _reactRouter.browserHistory.push('/home/' + _this2.state.restaurantId);
	      } else {
